...
Where a shell is available, `bin/kb-search.sh "<question>"` (or
`bin\kb-search.ps1` on Windows) does the same thing with ranking, and exits 1
when nothing matches. Where it is not, grep is entirely sufficient; this skill
...
that question already exists. Where a shell is available,
`bin/kb-gap.sh --question "<their question>"` does both and handles the
slug and the counter; otherwise write or edit the file directly — the format is
in `articles/README.md`.
